-- ================================================================
-- CMMS: FIX EXISTING FUNCTIONS - Set row_security = OFF (Using ALTER)
-- ================================================================
-- The SET row_security = OFF in CREATE FUNCTION doesn't persist visibly.
-- We'll use ALTER FUNCTION to explicitly set it after creation.
...
DECLARE
  ...
  v_user_id uuid;
BEGIN
  -- Attempt to get current user ID (may be NULL in some contexts)
  v_user_id := auth.uid();
...
-- ================================================================
-- ALTER FUNCTIONS TO DISABLE ROW SECURITY
-- ================================================================
-- Use ALTER FUNCTION with SET row_security = OFF configuration

ALTER FUNCTION public.fn_create_cmms_department(uuid, character varying, text, character varying) 
  SET row_security = OFF;

ALTER FUNCTION public.fn_update_cmms_department(uuid, character varying, text, character varying, boolean) 
  SET row_security = OFF;

ALTER FUNCTION public.fn_delete_cmms_department(uuid) 
  SET row_security = OFF;
...
ALTER FUNCTION public.fn_update_cmms_company(uuid, character varying, character varying, character varying, character varying, character varying) 
  SET row_security = OFF;

ALTER FUNCTION public.fn_get_cmms_company(uuid) 
  SET row_security = OFF;
...
ALTER FUNCTION public.fn_get_departments_by_company(uuid) 
  SET row_security = OFF;
...
SELECT 'All CMMS functions created and RLS bypass configured!' as status;

-- Check function configuration
SELECT 
  f.proname as function_name,
  pg_get_function_identity_arguments(f.oid) as arguments,
  CASE 
    WHEN (SELECT array_to_string(proconfig, ',') FROM pg_proc WHERE oid = f.oid) LIKE '%row_security=off%' 
    THEN true 
    ELSE false 
  END as has_rls_bypass
FROM pg_proc f
WHERE f.pronamespace = 'public'::regnamespace
  AND (f.proname LIKE 'fn_create_cmms_%'
   OR f.proname LIKE 'fn_update_cmms_%'
   OR f.proname LIKE 'fn_delete_cmms_%'
   OR f.proname LIKE 'fn_get_cmms_%'
   OR f.proname LIKE 'fn_get_departments%')
ORDER BY f.proname;
