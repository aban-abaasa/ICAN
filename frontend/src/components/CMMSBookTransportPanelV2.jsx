...
export default function CMMSBookTransportPanel({ companyProfile, canCreate = false, canEdit = false, canAssign = false }) {
  ...
  const [workerAllowJourneys, setWorkerAllowJourneys] = useState(false);
  const [workerJourneyLimit, setWorkerJourneyLimit] = useState('');
  ...
  const allocateWorker = async event => {
    ...
    const journeyLimit = workerAllowJourneys && workerJourneyLimit !== '' ? Number(workerJourneyLimit) : null;
    if (journeyLimit !== null && !(journeyLimit > 0)) { setError('Enter a journey limit above zero, or leave it empty for no limit.'); setSaving(false); return; }
    const { error: allocationError } = await supabase.rpc('mbg_allocate_company_transport_worker', {
      ...
      // Sent only when switched on, so assigning ride transport still works where the
      // journey-payment SQL (mybodaguy ADD_JOURNEY_COMPANY_PAYMENT.sql) isn't installed yet.
      ...(workerAllowJourneys ? { p_allow_journeys: true, p_journey_limit_ican: journeyLimit } : {}),
    });
    if (allocationError) setError(allocationError.message);
    else { setWorkerEmail(''); setWorkerEndsAt(''); setWorkerDailyStart('06:00'); setWorkerDailyEnd('18:00'); setWorkerAllowJourneys(false); setWorkerJourneyLimit(''); setMessage('Employee transport benefit activated. The employee can now book from BodaGo/Supermarkera with Company payment.'); }
    setSaving(false);
  };
