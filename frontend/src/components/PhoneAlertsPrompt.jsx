...
import {
  ...
  refreshWalletPhoneAlerts,
} from '../services/walletPushService';

const SNOOZE_KEY = 'icanera-push-prompt-snoozed-until';
...
/**
 * Asks a signed-in person, once, to turn on phone alerts so wallet, business and
 * CMMS notifications reach them while the app is closed. Silent for people who
 * already have alerts on (it just re-attaches their device to whoever is signed
 * in), on browsers that cannot do push, and after a "Not now" for a few days.
 * Sits at the top so it never covers the install prompt at the bottom.
 */
export default function PhoneAlertsPrompt() {
  ...
  const evaluate = useCallback(async (hasSession) => {
    ...
    try {
      ...
      if (status.enabled) { setVisible(false); refreshWalletPhoneAlerts(); return; }
      setVisible(status.supported && Notification.permission === 'default' && !snoozed());
    } catch {
      setVisible(false);
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="font-semibold text-slate-900">Never miss a payment or an alert</p>
        <p className="text-xs text-slate-600">Get wallet, business and workspace alerts on this device — even when the app is closed.</p>
