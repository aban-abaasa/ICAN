...
import { Search, Copy, Check, Loader, Store, TrendingUp, Package, ExternalLink } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { getDropshippableProducts, setDropshipListing, getResellerDropshipSales } from '../services/dropshipService';
...
const DropshipResellerDashboard = ({ businessProfileId }) => {
  ...
  const handleList = async (product) => {
    ...
    const { error } = await setDropshipListing(businessProfileId, product.product_id, price, true);
  ...
  const handleUnlist = async (product) => {
    setSavingId(product.product_id);
    const { error } = await setDropshipListing(businessProfileId, product.product_id, product.listed_price, false);
    if (!error) loadProducts(query);
    setSavingId(null);
  };
