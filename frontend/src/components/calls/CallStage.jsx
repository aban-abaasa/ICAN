...
// Zoom-style toolbar button: icon in a circle with a label underneath.
...
/**
 * Full-page video call view — Zoom/YouTube-Live styled: a "LIVE"/"RINGING"
 * badge, a name tag over the video, and a translucent bottom toolbar with
 * labeled controls. Takes over the whole widget (header, tabs, messages,
 * composer all hidden underneath) for as long as a video call is
 * ringing-out or active — actual room to see the other person instead of a
 * postage-stamp thumbnail. Audio-only calls and an incoming ring (no camera
 * yet — media isn't requested until Accept) stay on the slim `CallDock`
 * bar instead; this never renders for those states.
 */
const CallStage = ({ call }) => {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col bg-black">
      <div className="relative flex-1 bg-slate-900">
        {hasRemote ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-white">
            <span className={`flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-2xl font-bold ${isRinging ? 'animate-pulse' : ''}`}>
        ...
        {/* Top bar: YouTube-Live-style badge + elapsed/ringing time */}
        ...
        {/* Peer name tag over the video, Zoom-style */}
      ...
      {/* Zoom-style translucent bottom toolbar */}
