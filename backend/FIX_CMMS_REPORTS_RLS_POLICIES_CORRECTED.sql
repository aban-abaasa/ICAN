-- ============================================================
-- FIX: CMMS Company Reporting System - CORRECTED VERSION
-- ============================================================
-- This fixes the RLS policies to allow:
-- - ADMIN: view ALL reports from entire company
-- - FINANCE: view ALL reports from entire company
-- - COORDINATOR/SUPERVISOR: view departmental + own reports
-- - REGULAR USERS (member): view only their own reports
-- ============================================================
-- FIXES: Properly qualified column references (no ambiguous columns)
-- ============================================================
...
FOR SELECT USING (
  ...
  AND EXISTS (
    ...
    WHERE au.id = auth.uid()
      ...
      AND (
        -- Admin and Finance: see all reports from their company
...
-- ============================================================
-- Updated RPC Function with Role-Based Access (CORRECTED)
-- ============================================================
...
BEGIN
  ...
  -- Get user role, id, and department
  ...
  WHERE r.cmms_company_id = p_company_id
    AND (
      -- Admin and Finance: see ALL reports from company
      ...
      OR (
        ...
        AND r.reporter_cmms_user_id = v_user_id
      )
    )
...
-- ============================================================
-- RPC: Get Reports with Filters (Admin/Finance friendly)
-- ============================================================
...
-- ============================================================
-- RPC: Get Reports by Department (for coordinators/supervisors)
-- ============================================================
...
-- ============================================================
-- RPC: Export Report as Text/PDF-friendly format
-- ============================================================
...
-- ============================================================
-- RPC: Get All Reports (for Admin only)
-- ============================================================

CREATE OR REPLACE FUNCTION public.fn_get_all_company_reports(p_company_id UUID)
RETURNS TABLE (
  id UUID,
  cmms_company_id UUID,
  department_id UUID,
...
BEGIN
  ...
  -- Get user role
  SELECT COALESCE(cu.role, 'member')
  INTO v_user_role
  FROM public.cmms_users cu
  WHERE cu.cmms_company_id = p_company_id
    AND LOWER(cu.email) = LOWER(v_auth_email)
  ...
  -- Only admin and finance can see all reports
  IF LOWER(v_user_role) NOT IN ('admin', 'finance') THEN
    RAISE EXCEPTION 'Only admin or finance can view all reports';
  END IF;

  -- Return ALL reports from company
  RETURN QUERY
  SELECT r.id, r.cmms_company_id, r.department_id, r.report_title, 
         r.report_category, r.severity, r.report_body, r.reporter_cmms_user_id,
         r.reporter_name, r.reporter_email, r.reporter_role, r.status,
...
GRANT EXECUTE ON FUNCTION public.fn_get_all_company_reports TO authenticated;

SELECT 'CMMS Reports RLS Policies Fixed - Corrected Version!' AS status;
