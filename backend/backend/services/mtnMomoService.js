...
class MTNMomoService {
   ...
   * Falls back to environment variables if Supabase connection fails
   */
  async initializeFromSupabase() {
    try {
      console.log('🔍 Attempting to load MOMO config from Supabase...');
      ...
      console.warn('⚠️  Error loading Supabase config:', error.message);
      console.log('✅ Using environment variables as fallback');
    }
  }
  ...
  async logTransaction(transactionData) {
    try {
      // Only log the fields that exist in the table schema
      const logData = {
        ...
        request_payload: transactionData.requestPayload || null,
        response_payload: transactionData.responsePayload || null,
        error_message: transactionData.errorMessage || null,
        http_status_code: transactionData.httpStatus || null,
        endpoint: transactionData.endpoint || '/momo/collection',
        request_method: 'POST'
      };
      ...
      if (error) {
        console.warn(`⚠️  Failed to log transaction to Supabase:`, error.message);
      ...
      // Silently fail - don't block the main transaction
    }
  }
